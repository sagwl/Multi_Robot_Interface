...
{
    selected_dictionary selected_list;
    RaycastHit hit;
    bool dragSelect;
    ...
    {
        dragSelect = false;
        selected_list = GetComponent<selected_dictionary>();
        ...
        if (Input.GetMouseButtonUp(0))
        {
            if (dragSelect == false)
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    {
                        selected_list.addSelected(hit.collider.gameObject);
                    }
                    else // exclusive selection
                    {
                        selected_list.deselectAll();
                        selected_list.addSelected(hit.collider.gameObject);
                    }
                }
            ...
            else // drag select is true in this case
            {
                if (Input.GetKey(KeyCode.LeftShift))
                {
                    ...
                    foreach (GameObject item in allObjects)
                    {
                        ...
                        {
                            selected_list.addSelected(item);
                        }
                    }
                }
                else
